...
export default function BallotPage() {
  ...
  useEffect(() => {
    const u = GTV.getCurrentUser()
    if (!u) { navigateTo('/login'); return }
    if (u.voted) { navigateTo('/dashboard'); return }
    setUser(u)
  }, [])
  ...
  function submitBallot() {
    if (!user) return
    const code = GTV.generateReceipt()
    const votes: Record<string, string> = {}
    POSITIONS.forEach((pos, i) => {
      if (selections[i] !== null) votes[pos.id] = pos.candidates[selections[i]!].name
    })
    GTV.saveBallot({ receiptCode: code, timestamp: new Date().toISOString(), votes })
    GTV.markVoted(user.id, code)
    sessionStorage.setItem('gt_receipt', code)
    setReceiptCode(code)
    setReviewOpen(false)
      ...
      <div className={`ballot-page${fadingOut ? ' fading-out' : ''}`}>
        ...
        <nav className="ballot-nav">
          <div className="ballot-nav-left">
            <img src="/gctu-crest.png" alt="GCTU" className="ballot-nav-crest" />
