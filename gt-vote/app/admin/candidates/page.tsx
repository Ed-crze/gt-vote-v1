...
const ALL_CANDIDATES: CandidateRow[] = POSITIONS.flatMap(pos =>
  pos.candidates.map(c => ({ name: c.name, position: pos.title, faculty: c.faculty, level: c.faculty.split(' - ')[1] || '' }))
)
...
export default function AdminCandidatesPage() {
  ...
  const [candidates, setCandidates] = useState<CandidateRow[]>(ALL_CANDIDATES)
  ...
  function openEdit(i: number) {
    ...
    setFormName(c.name); setFormPosition(c.position); setFormFaculty(c.faculty); setFormLevel(c.level); setFormSlogan('')
    setModalOpen(true)
  }
  ...
  function saveCandidate() {
    ...
    if (editIndex !== null) {
      ...
      updated[editIndex] = { name: formName, position: formPosition, faculty: formFaculty, level: formLevel }
      setCandidates(updated)
      showToast('Candidate updated')
    } else {
      setCandidates([...candidates, { name: formName, position: formPosition, faculty: formFaculty, level: formLevel }])
