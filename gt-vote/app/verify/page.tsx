...
export default function VerifyPage() {
  ...
  useEffect(() => {
    ...
    const user = GTV.getCurrentUser()
    if (user?.receiptCode) setCode(user.receiptCode)
  }, [])
  ...
  function verify() {
    ...
    const ballot = GTV.findBallot(code)
    if (ballot) {
      const ts = ballot.timestamp
        ? new Date(ballot.timestamp).toLocaleString('en-GB', {
    ...
    <div className={`verify-page${fadingOut ? ' fading-out' : ''}`}>
      ...
      <nav className="verify-nav">
        <div className="verify-nav-left">
          <img src="/gctu-crest.png" alt="GCTU" className="verify-nav-crest" />
